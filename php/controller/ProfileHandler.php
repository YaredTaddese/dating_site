...
class ProfileHandler{
    ...
    public function advancedSearch($currentSIndex=0,$number=6){
         if(empty($_GET['initialAge']) && empty($_GET['finalAge']) && empty($_GET['initialHeight']) && empty($_GET['finalHeight'])
         && empty($_GET['religion']) && empty($_GET['mStatus']) && empty($_GET['location']) && empty($_GET['build'])
         && empty($_GET['education']) && empty($_GET['occupation']) && empty($_GET['drinking']) && empty($_GET['smoking'])
         && empty($_GET['haveChildren'])){
             return null;
         }
        ...
        if(!empty($_GET['initialAge'])){
            ...
            $initialStamp = strftime("%Y-%m-%d", $initialStamp);
            $query .= " and profile.birthday < '$initialStamp'";
        }
        if(!empty($_GET['finalAge'])){
            if(!empty($_GET['initialAge']) && (intval($_GET['finalAge']) < intval($_GET['initialAge']))){
            ...
            $finalStamp = strftime("%Y-%m-%d", $finalStamp);
        ...
        $result = $db->query($query);
        if(!$result->num_rows){
            return array();       //note that this can also run because of database query is not executed successfully 
        }
    ...
    public function getSuggestedProfiles($currentSIndex=0,$number=10){
        ...
        if(!empty($self->ageRange[0])){
            ...
            $initialStamp = time() - $initialAge;
            $query .= " or profile.birthday < $initialStamp"; 
        }
        if(!empty($self->ageRange[1])){
            ...
            $finalStamp = time() - $finalAge;
            $query .= " or profile.birthday > $finalStamp"; 
        }
        ...
        if(!empty($self->location)){
            ...
            $query .= " or profile.location = '$location'";
        }
