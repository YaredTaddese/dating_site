<?php
    ...
    $screenNameError='';$nameError='';$profilePhotoError='';$phoneNumberError='';
    $isUpdated = "no";
    ...
    else if(isset($_GET['moreUpdate'])){
        $profileInfo = $self->profileInfo;
        $profileInfo->mStatus = $_GET['mStatus'];
        $profileInfo->religion = $_GET['religion'];
        $profileInfo->haveChildren = $_GET['haveChildren'];
        $profileInfo->height = $_GET['height'];
        $profileInfo->build = $_GET['build'];
        $profileInfo->education = $_GET['education'];
        $profileInfo->occupation = $_GET['occupation'];
        $profileInfo->smoking = $_GET['smoking'];
        $profileInfo->drinking = $_GET['drinking'];
        if(!empty($_GET['phoneNumber'])){
            if(!mb_ereg("(\+[[:digit:]]{3})?[[:digit:]]{9,10}",$_GET['phoneNumber'])){
                $phoneNumberError ="Invalid phone number" ;
            }
            else{
                $profileInfo->phoneNumber = $_GET['phoneNumber'];
            }
        }
        if(empty($phoneNumberError)){
            $isUpdated = $profileInfo->update();
        }
        else{
            $isUpdated = false;
        }
    }
    else if(isset($_GET['criteriaUpdate'])){
        $criteria = $self->criteria;
        $criteria->mStatus = $_GET['mStatus'];
        $criteria->religion = $_GET['religion'];
        $criteria->haveChildren = $_GET['haveChildren'];
        $criteria->build = $_GET['build'];
        $criteria->education = $_GET['education'];
        $criteria->location = $_GET['location'];
        $criteria->occupation = $_GET['occupation'];
        $criteria->smoking = $_GET['smoking'];
        $criteria->drinking = $_GET['drinking'];
        if(empty($_GET['initialAge']) && empty($_GET['finalAge'])){
            $criteria->ageRange = array();
        }
        else{
            $criteria->ageRange[0] = (empty($_GET['initialAge'])) ? "" : $_GET['initialAge'];
            $criteria->ageRange[1] = (empty($_GET['finalAge'])) ? "" : $_GET['finalAge'];
        }
        if(empty($_GET['initialHeight']) && empty($_GET['finalHeight'])){
            $criteria->heightRange = array();
        }
        else{
            $criteria->heightRange[0] = (empty($_GET['initialHeight'])) ? "" : $_GET['initialHeight'];
            $criteria->heightRange[1] = (empty($_GET['finalHeight'])) ? "" : $_GET['finalHeight'];
        }
        
        $isUpdated = $criteria->update();
        if($isUpdated != true){
            $isUpdated = false;
        }
    }
...
<?php   
    ...
    if(!empty($_GET['additional'])){
         $self->profileInfo->viewAsEditable($phoneNumberError);
    }
