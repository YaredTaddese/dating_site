...
<?php
    ...
    if(!empty($interactor)){
        echo "<p id='loaderEmail' style='display:none'>$self->email</p>";
        echo "<p id='activeEmail' style='display:none'>$interactor->email</p>";
        /*
        if(!empty($_GET['more']) && !empty($_GET['index'])){
            $jsIndex = $_GET['index'];
            echo "<p id='jsIndex' style='display:none'>$jsIndex</p>";
            echo "<p id='jsMore' style='display:none'>yes</p>";
        }
        */
        //echo "<script src='../../js/seen.js'></script>";
    ...
    <?php
        if(empty($_REQUEST['email'])){
            if(empty($unseenMessages)){
                echo'
                <div class="postMessage" id="noMessages" style="text-align:center;color:#333;font-size:1.2em;width:32em;">
                    <p class="postText" > 
                        No new Messages
                    </p>
                </div>  ';
    ...
    if(!empty($_GET['more']) && !empty($_GET['index'])){
       ...
       if(empty($profiles)){
            ...
            foreach($nemails as $nemail){
                    ...
                    $ph->init($nprofile);
                if(empty($interactor) || ($profile->email != $interactor->email)){
