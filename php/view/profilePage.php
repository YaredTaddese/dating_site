...
<?php
    if(isset($_GET['more'])){
        $posts = $mh->getPostMessages($self->email,true,$_GET['index']);
        $notification = "You have seen all posts of your nominees.<br/>
                         Please, add peoples you are interested in to your nominees list to see their posts";
    } 
    else{
        $posts = $mh->getPostMessages($self->email);
        $notification = "No posts available!
                        <br/>Please, add peoples you are interested in to your nominees list to see their posts";
    }
    if(empty($posts)){
        echo'
        ...
        echo"
        <a class='more' href='profilePage.php?more=yes&index=$mh->currentPIndex'>
            See More
        </a>
        ";  
    }   
